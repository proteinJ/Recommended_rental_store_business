...
function setPopupInfo(군구, row) {
  ...
  // 주소 (이 부분은 기존 HTML에 따라 다를 수 있으니 확인해주세요)
  ...
  if (yearTableDiv && row) {
      ...
      // 가격을 10000으로 나누고 소수점 첫째 자리까지 포매팅 및 "억원" 추가
      const formattedPrices = years.map(y => {
          let price = parseFloat(row[`${y}`]); // 해당 연도의 가격 컬럼명을 정확히 사용
          if (isNaN(price)) {
              return '';
          }
      ...
      let tableHtml = "<table class='year-table' style='width:100%;text-align:center;'><tr>";
      // 연도 뒤에 "년" 추가 (이전과 동일)
      tableHtml += years.map(y => `<th>${y}년</th>`).join('');
      tableHtml += "</tr>";

      tableHtml += "<tr>";
      // 포매팅된 가격 출력
      tableHtml += formattedPrices.map(p => `<td>${p}</td>`).join('');
      tableHtml += "</tr></table>";
